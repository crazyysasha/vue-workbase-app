<template>
  <div class="relative">
    <div
      class="top-0 absolute w-full bg-white rounded-3xl overflow-hidden -z-10"
      :style="{
    ...
    >
      <ul :class="{ hidden: !isOpen }">
        ...
        <li class="py-3 px-4" v-else>По вашему запросу ничего не найдено</li>
      </ul>
    </div>
    <div class="group z-30" ref="searchContainer">
      <input
        ...
        "
        @keypress.enter="data?.data.length > 0 ? navigateTo(data.data[0]) : ''"
        @click="search.length > 0 ? (isOpen = true) : ''"
        :class="{
